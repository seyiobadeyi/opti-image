...
export function EnhanceTool({ embedded = false }: { embedded?: boolean }): React.JSX.Element {
        ...
        try {
            for (let i = 0; i < entries.length; i++) {
                ...
                const resp = await apiClient.convertImages([namedFile], {
                    autoEnhance,
                    exposure: autoEnhance ? undefined : exposure,
                    saturation: autoEnhance ? undefined : saturation,
            ...
            setSummary({ filesProcessed: allResults.length, totalOriginalSize: totalOrig, totalProcessedSize: totalProc, totalSavings: 0, totalSavingsPercent: '0.0' });
        } catch (err) {
    ...
    }, [entries, processing, autoEnhance, exposure, saturation, format]);
                ...
                {entries.length > 0 && !results && (
                    <>
                        ...
                        <div style={S.settings}>
                            <div style={S.sRow}>
                                <div>
                                    <div style={S.sLabel}>Auto enhance</div>
                                    <div style={S.sNote}>Automatically correct colour and brightness</div>
                                </div>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                    <input type="checkbox" checked={autoEnhance} onChange={e => setAutoEnhance(e.target.checked)}
                                        style={{ accentColor: clr.accent, width: '16px', height: '16px', cursor: 'pointer' }} />
                                    <span style={{ fontSize: '0.88rem', color: clr.g700 }}>{autoEnhance ? 'On' : 'Off'}</span>
                                </label>
                            </div>
                            ...
                            {!autoEnhance && (
                                <>
                                    ...
                                    <div style={S.sRow}>
                                        <div>
                                            <div style={S.sLabel}>Exposure / brightness</div>
                                            <div style={S.sNote}>{fmtSlider(exposure)}</div>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <input type="range" min={0.5} max={2.0} step={0.05} value={exposure} onChange={e => setExposure(Number(e.target.value))}
                                                style={{ width: '160px', accentColor: clr.accent, cursor: 'pointer' }} />
                                            <span style={{ fontSize: '0.88rem', fontWeight: 700, color: clr.accent, minWidth: '42px', textAlign: 'right' }}>{exposure.toFixed(2)}x</span>
                                        </div>
                                    </div>
                                    ...
                                    <div style={S.sRow}>
                                        <div>
                                            <div style={S.sLabel}>Saturation / colour</div>
                                            <div style={S.sNote}>{fmtSlider(saturation)}</div>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <input type="range" min={0.0} max={2.0} step={0.05} value={saturation} onChange={e => setSaturation(Number(e.target.value))}
                                                style={{ width: '160px', accentColor: clr.accent, cursor: 'pointer' }} />
                                            <span style={{ fontSize: '0.88rem', fontWeight: 700, color: clr.accent, minWidth: '42px', textAlign: 'right' }}>{saturation.toFixed(2)}x</span>
                                        </div>
                                    </div>
                                </>
                            )}
                            ...
                            <div style={S.sRow}>
                                <div>
                                    ...
                                    <div style={S.sNote}>Keep original format or convert</div>
                                </div>
                        ...
                        <button type="button" disabled={processing} onClick={handleProcess}
                            ...
                            {processing ? `Enhancing... ${processed} / ${entries.length}` : `Enhance ${entries.length} image${entries.length !== 1 ? 's' : ''}`}
                        </button>
                ...
                {results && summary && (
                    <>
                        <div style={S.banner}>
                            ...
                            <div style={S.bannerLbl}>{results.length} {results.length === 1 ? 'photo' : 'photos'} enhanced</div>
                            <div style={S.bannerSub}>{results.length} {results.length === 1 ? 'file' : 'files'} ready to download</div>
                        </div>

                        <button type="button" style={S.dlAllBtn} onClick={downloadAll}
                            onMouseEnter={e => (e.currentTarget.style.background = clr.accentDk)}
                            onMouseLeave={e => (e.currentTarget.style.background = clr.accent)}>
                            <Download size={16} /> Download {results.length > 1 ? `all ${results.length} as ZIP` : 'enhanced file'}
                        </button>
                        ...
                        <button type="button" style={S.resetBtn} onClick={reset}>← Enhance more images</button>
                    </>
                )}
            </div>
