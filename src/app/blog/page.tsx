...
export const metadata: Metadata = {
    title: 'The Optimage Journal: Web Performance Insights',
    ...
    return (
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '120px 24px 80px' }}>
            ...
            <div style={{ maxWidth: '680px', marginBottom: '60px' }}>
                ...
                <p style={{ fontSize: '1.1rem', color: '#374151', lineHeight: 1.65 }}>
            ...
            {posts.length === 0 ? (
                <div style={{ padding: '40px', background: '#ffffff', borderRadius: '16px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                    <h3 style={{ marginBottom: '16px' }}>Content Generating...</h3>
                    <p style={{ color: '#374151' }}>New articles are on the way. Check back soon.</p>
                </div>
            ) : (
                <>
                    {/* Card grid */}
                    ...
                    {totalPages > 1 && (
                        <div style={{
                            ...
                            {safePage > 1 ? (
                                ...
                                <span style={{ padding: '8px 18px', borderRadius: '10px', border: '1px solid #e5e7eb', color: '#9ca3af', fontSize: '0.88rem', opacity: 0.35 }}>
                                    ← Previous
                                </span>
                            )}
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => {
                                ...
                                if (showEllipsis) return <span key={`e-${page}`} style={{ color: '#9ca3af', fontSize: '0.88rem' }}>…</span>;
                                if (!show) return null;
                                return (
                                    <Link
                                        key={page}
                                        ...
                                        style={{
                                            ...
                                            border: `1px solid ${isActive ? '#db5a42' : '#e5e7eb'}`,
                                            background: isActive ? '#db5a42' : 'transparent',
