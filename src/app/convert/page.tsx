...
const S: Record<string, React.CSSProperties> = {
    ...
    formatPill: { fontSize: '0.72rem', fontWeight: 700, padding: '2px 8px', borderRadius: '999px', background: clr.accentLt, color: clr.accent, whiteSpace: 'nowrap' as const, textTransform: 'uppercase' as const },
...
const FORMAT_OPTIONS = [
    { value: 'webp',  label: 'WebP', note: 'Best for web — smallest size' },
    { value: 'avif',  label: 'AVIF', note: 'Next-gen format, very small' },
    { value: 'jpeg',  label: 'JPEG', note: 'Universal compatibility' },
    { value: 'png',   label: 'PNG',  note: 'Lossless, supports transparency' },
    { value: 'tiff',  label: 'TIFF', note: 'High quality for print' },
        ...
        try {
            for (let i = 0; i < entries.length; i++) {
                ...
                if (resp.success && resp.results[0]) {
                    ...
                    allResults.push({ ...r, displayName: `${entry.name}.${format}`, localUrl: entry.localUrl });
    ...
    return (
        <div style={S.page}>
            ...
            <div style={S.tool}>
                ...
                <div style={{ marginBottom: '20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '10px' }}>
                    {FORMAT_OPTIONS.map(opt => (
                        <button key={opt.value} type="button" onClick={() => setFormat(opt.value)}
                            style={{
                                ...
                                border: `2px solid ${format === opt.value ? clr.accent : clr.g200}`,
                                background: format === opt.value ? clr.accentLt : '#fff',
                                transition: 'border-color 0.15s, background 0.15s',
                            }}>
                            <div style={{ fontSize: '0.9rem', fontWeight: 700, color: format === opt.value ? clr.accent : clr.g900 }}>{opt.label}</div>
                            <div style={{ fontSize: '0.72rem', color: clr.g400, marginTop: '2px' }}>{opt.note}</div>
                        </button>
                    ))}
                </div>
                ...
                {entries.length > 0 && !results && (
                    <>
                        <div style={S.fileList}>
                            {entries.map((entry, i) => (
                                <div key={i} style={{ ...S.fileRow, background: i % 2 === 0 ? '#fff' : clr.g50 }}>
                                    ...
                                    <span style={{ ...S.fsize, marginRight: '4px' }}>→ <strong>.{format}</strong></span>
                        ...
                        <button type="button" disabled={processing} onClick={handleProcess}
                            ...
                            {processing ? `Converting... ${processed} / ${entries.length}` : `Convert ${entries.length} image${entries.length !== 1 ? 's' : ''} to ${format.toUpperCase()}`}
                        </button>
                ...
                {results && summary && (
                    <>
                        <div style={S.banner}>
                            ...
                            <div style={S.bannerLbl}>{results.length} {results.length === 1 ? 'image' : 'images'} converted to {format.toUpperCase()}</div>
                            <div style={S.bannerSub}>{fmtBytes(summary.totalOriginalSize)} → {fmtBytes(summary.totalProcessedSize ?? 0)}</div>
                        </div>

                        <button type="button" style={S.dlAllBtn} onClick={downloadAll}
                            onMouseEnter={e => (e.currentTarget.style.background = clr.accentDk)}
                            onMouseLeave={e => (e.currentTarget.style.background = clr.accent)}>
                            <Download size={16} /> Download {results.length > 1 ? `all ${results.length} as ZIP` : 'converted file'}
                        </button>
                        ...
                        <div style={S.resList}>
                            {results.map((r, i) => (
                                <div key={r.id} style={{ ...S.resRow, background: i % 2 === 0 ? '#fff' : clr.g50 }}>
                                    ...
                                    <span style={S.formatPill}>{format}</span>
                        ...
                        <div style={{ marginTop: '16px', padding: '14px 16px', background: '#fdf3f1', border: '1px solid #fce4dc', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                            <div>
                                <div style={{ fontSize: '0.88rem', fontWeight: 600, color: clr.g900 }}>Save your conversion history</div>
                                <div style={{ fontSize: '0.78rem', color: clr.g400, marginTop: '2px' }}>Free account — track every file you&apos;ve converted.</div>
                            </div>
                            <button type="button"
                                onClick={() => window.dispatchEvent(new Event('open-auth-modal'))}
                                style={{ background: clr.accent, color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 18px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background 0.15s' }}
                                onMouseEnter={e => (e.currentTarget.style.background = clr.accentDk)}
                                onMouseLeave={e => (e.currentTarget.style.background = clr.accent)}>
                                Sign up free
                            </button>
                        ...
                        <button type="button" style={S.resetBtn} onClick={reset}>← Convert more images</button>
                    </>
                )}
            </div>
