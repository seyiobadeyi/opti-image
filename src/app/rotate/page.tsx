...
export default function RotatePage(): React.JSX.Element {
    ...
    const handleProcess = useCallback(async () => {
        ...
        try {
            ...
            setSummary({ filesProcessed: allResults.length, totalOriginalSize: totalOrig, totalProcessedSize: totalProc, totalSavings: totalOrig - totalProc, totalSavingsPercent: '0.0' });
        } catch (err) {
    ...
    }, [entries, processing, rotate, flipH, flipV, format]);
                ...
                {entries.length > 0 && !results && (
                    <>
                        ...
                        <div style={S.settings}>
                            ...
                            <div style={S.sRow}>
                                <div>
                                    <div style={S.sLabel}>Rotation</div>
                                    <div style={S.sNote}>Clockwise rotation angle</div>
                                </div>
                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    {ROTATE_PRESETS.map(p => (
                                        <button key={p.value} type="button" onClick={() => setRotate(p.value)}
                                            style={{ padding: '6px 14px', borderRadius: '8px', border: `1.5px solid ${rotate === p.value ? clr.accent : clr.g200}`, background: rotate === p.value ? clr.accentLt : '#fff', color: rotate === p.value ? clr.accent : clr.g700, fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
                                            {p.label}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div style={S.sRow}>
                                <div>
                                    <div style={S.sLabel}>Mirror / flip</div>
                                    <div style={S.sNote}>Can be combined with rotation</div>
                                </div>
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    <button type="button" onClick={() => setFlipH(v => !v)}
                                        style={{ padding: '6px 14px', borderRadius: '8px', border: `1.5px solid ${flipH ? clr.accent : clr.g200}`, background: flipH ? clr.accentLt : '#fff', color: flipH ? clr.accent : clr.g700, fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
                                        Flip H
                                    </button>
                                    <button type="button" onClick={() => setFlipV(v => !v)}
                                        style={{ padding: '6px 14px', borderRadius: '8px', border: `1.5px solid ${flipV ? clr.accent : clr.g200}`, background: flipV ? clr.accentLt : '#fff', color: flipV ? clr.accent : clr.g700, fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
                                        Flip V
                                    </button>
                                </div>
                            </div>
                        ...
                        <button type="button" disabled={processing} onClick={handleProcess}
                            ...
                            {processing ? `Processing... ${processed} / ${entries.length}` : `${actionLabel} ${entries.length} image${entries.length !== 1 ? 's' : ''}`}
                        </button>
                ...
                {results && summary && (
                    <>
                        <div style={S.banner}>
                            ...
                            <div style={S.bannerLbl}>{results.length} {results.length === 1 ? 'image' : 'images'} processed</div>
                        ...
                        <button type="button" style={S.dlAllBtn} onClick={downloadAll}
                            onMouseEnter={e => (e.currentTarget.style.background = clr.accentDk)}
                            onMouseLeave={e => (e.currentTarget.style.background = clr.accent)}>
                            <Download size={16} /> Download {results.length > 1 ? `all ${results.length} as ZIP` : 'processed file'}
                        </button>
                        ...
                        <button type="button" style={S.resetBtn} onClick={reset}>← Rotate more images</button>
                    </>
                )}
            </div>
