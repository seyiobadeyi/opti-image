...
featured: false
variants:
  - excerpt: "WordPress 6.8 adds AVIF upload and generation support, separate quality settings for WebP (default 82) and AVIF (default 65), and a new Performance Mode that enables lazy loading, progressive WebP generation, and fetchpriority='high' on first images — all in one toggle."
    keyTakeaways:
      - "AVIF support requires PHP 8.1+, GD with libavif/libaom, or ImageMagick 7.0.25+ — check your host first"
      - "WordPress 6.8 WebP default quality: 82; AVIF default quality: 65 — both are good starting points"
      - "Progressive WebP generation means WebP files are created on-demand, not immediately on upload"
      - "2048x2048 size is now optional (not generated by default) — reduces storage and processing overhead"
  - excerpt: "The biggest 6.8 change for existing sites: WebP generation via the image_editor_output_format filter is now applied consistently across all image generation contexts. If you previously set up this filter but noticed inconsistent WebP output, 6.8 fixes it — but also means any workarounds to avoid WebP generation need review."
    keyTakeaways:
      - "ShortPixel, Imagify, Smush may conflict with WordPress 6.8 built-in WebP — check plugin changelogs"
      - "After upgrading: enable Performance Mode, regenerate thumbnails, verify WebP delivery in Chrome DevTools"
      - "WooCommerce thumbnail regeneration is required after 6.8 upgrade for consistent image sizes"
      - "New medium_large_portrait size (576x768px) added for portrait-orientation content on mobile"
  - excerpt: "For small sites with modest traffic, WordPress 6.8's built-in capabilities may be sufficient for image optimization. For high-traffic sites or e-commerce, a hybrid approach works best: let WordPress handle WebP for standard content, use external tools for hero images and product photos, use a CDN for AVIF delivery."
    keyTakeaways:
      - "Back up wp-content/uploads before upgrading — full backup, not just database"
      - "Test on staging for 2 weeks before upgrading production — especially if using page builders"
      - "Run wp_get_registered_image_subsizes() on staging to audit custom sizes before upgrade"
      - "A CDN with AVIF support (Cloudflare) handles browser compatibility transparently without WordPress changes"
---
...
<svg viewBox="0 0 700 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:700px;margin:0 auto;display:block">
  ...
  <g class="sb"><rect x="30" y="20" width="160" height="70" rx="12" fill="#fdf3f1"/><text x="110" y="58" text-anchor="middle" class="sn">40%</text><text x="110" y="78" text-anchor="middle" class="sl">Of all websites run WordPress</text></g>
  <g class="sb"><rect x="270" y="20" width="160" height="70" rx="12" fill="#fdf3f1"/><text x="350" y="58" text-anchor="middle" class="sn">q82</text><text x="350" y="78" text-anchor="middle" class="sl">Default WebP quality in WP 6.8</text></g>
  <g class="sb"><rect x="510" y="20" width="160" height="70" rx="12" fill="#fdf3f1"/><text x="590" y="58" text-anchor="middle" class="sn">q65</text><text x="590" y="78" text-anchor="middle" class="sl">Default AVIF quality in WP 6.8</text></g>
</svg>
</div>
...
<figure role="img" aria-label="WordPress 6.8 image processing workflow" style="margin:32px 0">
<svg viewBox="0 0 660 100" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:660px;display:block;margin:0 auto">
  ...
  <defs><marker id="arwp" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0L0,6L8,3z" fill="#d1d5db"/></marker></defs>
  <g class="px"><rect x="10" y="15" width="175" height="70" rx="12" fill="#fdf3f1" stroke="#f9c5b8" stroke-width="1.5"/><text x="97" y="46" text-anchor="middle" class="pn">① Upload</text><text x="97" y="66" text-anchor="middle" class="pt">JPEG / PNG / WebP / AVIF</text></g>
  <line x1="188" y1="50" x2="238" y2="50" stroke="#d1d5db" stroke-width="2" marker-end="url(#arwp)"/>
  <g class="px"><rect x="243" y="15" width="175" height="70" rx="12" fill="#fdf3f1" stroke="#f9c5b8" stroke-width="1.5"/><text x="330" y="46" text-anchor="middle" class="pn">② WP 6.8</text><text x="330" y="66" text-anchor="middle" class="pt">Progressive WebP + AVIF gen</text></g>
  <line x1="421" y1="50" x2="471" y2="50" stroke="#d1d5db" stroke-width="2" marker-end="url(#arwp)"/>
  <g class="px"><rect x="476" y="15" width="175" height="70" rx="12" fill="#fdf3f1" stroke="#f9c5b8" stroke-width="1.5"/><text x="563" y="46" text-anchor="middle" class="pn">③ Serve</text><text x="563" y="66" text-anchor="middle" class="pt">&lt;picture&gt; with srcset</text></g>
</svg>
</figure>
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Does WordPress 6.8 support AVIF images?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes, WordPress 6.8 adds AVIF upload and generation support, but it is conditional on server capabilities. AVIF requires PHP 8.1+, and either GD compiled with libavif/libaom or ImageMagick 7.0.25+ compiled with AVIF support. Managed hosts like WP Engine and Kinsta are adding AVIF support, but shared hosting varies. The AVIF generation feature is opt-in by default — check Media > Media Handling Settings after upgrading to see if your server supports it."
      }
    },
    {
      "@type": "Question",
      "name": "What is WordPress 6.8 Performance Mode?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Performance Mode is a new option in WordPress 6.8 Media Settings that applies recommended performance defaults in one toggle: WebP generation enabled, progressive WebP generation (on-demand not on-upload), fetchpriority='high' on the first image in post content, and loading='lazy' on all subsequent images. These defaults align WordPress with modern web performance best practices that plugins like WP Rocket and Perfmatters have implemented for years. Recommended for all new WordPress 6.8 installations."
      }
    },
    {
      "@type": "Question",
      "name": "Will my image optimization plugin conflict with WordPress 6.8?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Potentially, yes. Image optimization plugins like ShortPixel, Imagify, Smush, and EWWW Image Optimizer all intercept uploads and apply their own compression and format conversion. With WordPress 6.8 adding its own WebP and AVIF generation, both WordPress and the plugin may attempt to generate format-converted images, potentially creating duplicate files or conflicting outputs. Check your plugin's changelog for WordPress 6.8 compatibility announcements and test on a staging site before upgrading production."
      }
    },
    {
      "@type": "Question",
      "name": "What should I do before upgrading to WordPress 6.8?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Four steps before upgrading: (1) Back up your entire wp-content/uploads directory. (2) Check image optimization and page builder plugin changelogs for 6.8 compatibility. (3) Audit your registered image subsizes with wp_get_registered_image_subsizes() on staging — the 2048x2048 size is now optional and some other defaults change. (4) Test on a staging environment for at least two weeks before upgrading production, especially if using Elementor, Divi, or other page builders that have their own image handling."
      }
    }
  ]
}
